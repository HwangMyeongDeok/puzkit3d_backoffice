            ...
            <FormItem>
              ...
              <div className="flex flex-col gap-3 mt-2 border p-4 rounded-md bg-muted/30">
                {drives?.length ? drives.map((drive) => {
                  // ĐẢM BẢO LẤY ĐÚNG ID: Nếu API trả về drive bị lồng (ví dụ drive: { id: ... }) thì bóc nó ra
                  // Trong SpecificationsCard.tsx (Khoảng dòng 131)
                  const driveId = (drive as any).drive?.id || drive.id;
                  const driveName = (drive as any).drive?.name || drive.name;

                  const currentDrives = (field.value as DriveDetail[] | undefined) || [];

                  // THÊM String() ĐỂ ÉP KIỂU KHI SO SÁNH:
                  const activeDrive = currentDrives.find((d) => String(d.driveId) === String(driveId));
                  const isChecked = !!activeDrive;

                  return (
                    <div key={driveId} className="flex items-center gap-4">
                      <label className="flex items-center gap-2 cursor-pointer text-sm w-1/3">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.checked) {
                              updated.push({ driveId: driveId, quantity: 1 });
                            } else {
                              updated = updated.filter((d) => d.driveId !== driveId);
                            }
                            field.onChange(updated);
                          }}
                        />
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const updated: DriveDetail[] = currentDrives.map((d) =>
                              d.driveId === driveId ? { ...d, quantity: newQty } : d
                            );
                            field.onChange(updated);
                          }}
                        />
