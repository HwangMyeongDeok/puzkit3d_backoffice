...
export function PartnerProductCreatePage() {
  ...
  const [formData, setFormData] =
    useState<UpsertPartnerProductRequest>(initialForm);
  const [referencePriceInput, setReferencePriceInput] = useState("");
  const [quantityInput, setQuantityInput] = useState("0");

  const [thumbnailFile, setThumbnailFile] = useState<File | null>(null);
  const [previewFiles, setPreviewFiles] = useState<(File | null)[]>([
    null,
    null,
    null,
  ]);
  ...
  useEffect(() => {
    const fetchPartners = async () => {
      try {
        ...
        console.error(error);
        toast.error("Unable to load partners.");
      }
    };
  ...
  const handleChange = (
    field: keyof UpsertPartnerProductRequest,
    value: string
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const slug =
        slugifyFromName(safeName) || `partner-product-${Date.now()}`;
      ...
      const payload: UpsertPartnerProductRequest = {
        ...
        previewAsset: buildPreviewAsset(previewUrls),
        slug,
      ...
      toast.success("Partner product created successfully.");
      navigate("/partner-products");
    } catch (error: any) {
      console.error("[CreatePartnerProduct] submit failed", error);
      console.error(
        "[CreatePartnerProduct] response status",
        error?.response?.status
      );
      console.error(
        "[CreatePartnerProduct] response data",
        error?.response?.data
      );

      if (error?.response?.status === 409) {
        toast.error(
          "The slug or product already exists. Please use a different name or slug."
        );
        return;
      }

      toast.error("Failed to create partner product.");
    } finally {
      setSubmitting(false);
    }
  };
